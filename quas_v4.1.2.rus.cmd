...
@echo    [ на 94^%% может на минуту замререть или завершить процесс загрузки, это нормально ]
rem EndRusTextBlock
rem StartEngTextBlock
...
rem StartRusTextBlock
@echo  = Загружаем прошивку в шлем..
@echo    [ на 94^%% на минуту замрёт или завершит процесс загрузки, это нормально ]
rem EndRusTextBlock
rem StartEngTextBlock
rem @echo  = Loading firmware into the headset..
rem @echo    [ at 94^%% it may freeze for a minute or complete the loading process, this is normal ]
...
@echo       пока не найдет подходящий DNS сервер из 80 доступных в списке и не скачает обновления.
@echo.
...
rem @echo    Н. Remove installed shortcuts and files manually
rem @echo.
rem @echo.
rem @echo IMPORTANT:
rem @echo.
...
rem @echo    Registry settings removed
...
@echo.
rem StartRusTextBlock
@echo     +++ Шлем подключен,но драйверы не установлены  +++
@echo.
@echo.
@echo        Тем не менее, вы можете запустить программу,
@echo        но некоторые функции работать не будут.
...
@echo     T.  Запустить с таблицей
@echo     D.  Открыть Диспетчер устройств
@echo     I.  Перейти в меню установки драйверов
rem EndRusTextBlock
rem StartEngTextBlock
rem @echo     +++ Headset is connected, but drivers are not installed  +++
rem @echo.
rem @echo.
rem @echo     +++++ However, you can run the program,
rem @echo     but some features will not work.
...
@echo.
rem StartRusTextBlock
@echo     +++ Подключенное утройство не выглядит как шлем  +++
@echo.
